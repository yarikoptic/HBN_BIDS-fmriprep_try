...
BOLD runs were slice-time corrected to 0.351s (0.5 of slice acquisition range
0s-0.703s) using `3dTshift` from AFNI 20160207 [@afni, RRID:SCR_005927].
...
parameters) are estimated before any spatiotemporal filtering using
`mcflirt` [FSL 5.0.9, @mcflirt].
BOLD runs were slice-time corrected to 0.35s (0.5 of slice acquisition range
0s-0.7s) using `3dTshift` from AFNI 20160207 [@afni, RRID:SCR_005927].
The BOLD time-series were resampled onto the following surfaces
(FreeSurfer reconstruction nomenclature):
*fsaverage*.
The BOLD time-series (including slice-timing correction when applied)
